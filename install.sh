...
install_binary() {
    ...
    # Create convenient CLI symlink (decoupled-ai -> decoupled-ai-server)
    local cli_link="${INSTALL_DIR}/decoupled-ai"
    if [ ! -e "$cli_link" ]; then
        ln -sf "${BINARY_NAME}" "$cli_link"
        log_success "Created CLI symlink: decoupled-ai -> decoupled-ai-server"
    fi
}
...
create_config() {
    ...
    if [ ! -f "$config_file" ]; then
        cat > "$config_file" <<EOF
# DeCoupled-AI Configuration (flat format for server compatibility)
...
# Security hardening - compatible with static PIE binary
NoNewPrivileges=yes
PrivateTmp=yes
